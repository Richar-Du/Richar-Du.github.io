...
本项目的目的是使用深度学习算法，对人体的心电信号进行分类，判断出被测试者心跳是否正常，或患有什么样的心脏疾病，最终实现心电数据的**7分类**。整个项目是分为如下几个阶段完成的：
...
在深度学习模型方面，首先我们尝试了简单的卷积神经网络、CNNLSTM组合、SENet-LSTM等模型，最终使用SENet-LSTM对**7种不同的心脏疾病进行分类**，到达97%的准确率，比参考论文的准确率提高了2%左右，假阴性率大大降低。同时我们将模型部署到了raspberry zero上，实现了边缘计算。
